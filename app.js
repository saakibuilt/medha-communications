...
$("#message-area").addEventListener("click",async e=>{
  ...
  try{const mode=message.text.includes("🎥")?"video":"audio";activeCall=videoClient.call("default",message.callId);await activeCall.join();await activeCall.microphone.enable();if(mode==="video")await activeCall.camera.enable();showCallSurface(activeCall,"Join call · "+active.name,mode);toast("Connected to call")}catch(error){toast(error.message)}
});
...
async function leaveStreamCall(){stopRingTone();activeParticipantSubscription?.unsubscribe?.();activeParticipantSubscription=null;activeMediaUnbinders.forEach(unbind=>{try{unbind()}catch{}});activeMediaUnbinders=[];if(activeCall){try{await activeCall.leave()}catch{}activeCall=null}incomingCall=null;$("#call-dialog").close();$("#call-minimized").hidden=true;$("#call-participants").innerHTML=""}
