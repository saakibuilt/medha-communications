...
for(const vp of [{w:390,h:844,n:"iPhone"},{w:820,h:1180,n:"iPad"}]){
  ...
  const r=await page.evaluate(kb=>{
    ...
    /* The shell is sized to the visible height and never offset, so the band
       starts at the document top and its bottom edge is the keyboard. */
    ...
    return {bottom:Math.round(c.bottom),top:Math.round(c.top),keyboardTop:Math.round(keyboardTop),
      ...
      atTop:c.top<window.visualViewport.height*0.35,
