...
create or replace function public.medha_communications_message_notification_trigger() returns trigger language plpgsql security definer set search_path = public, extensions as $$
declare recipient text; recipients text[]; sender_name text; normalized_words text[]; notification_text text;
begin
  select participant_ids into recipients from public.medha_communications_conversations where id=new.conversation_id;
  select coalesce(full_name,'New message') into sender_name from public.users where id=new.sender_id limit 1;
  normalized_words:=regexp_split_to_array(regexp_replace(trim(new.body),'\s+',' ','g'),' ');
  notification_text:=array_to_string(normalized_words[1:10],' ')||case when coalesce(array_length(normalized_words,1),0)>10 then ' …' else '' end;
    ...
    if recipient is not null and recipient<>'' and recipient<>new.sender_id then
      perform net.http_post(
        ...
        body := jsonb_build_object('source','space-message','event',jsonb_build_object(
          'activity_id',new.id,'recipient_uid',recipient,'event_type','space_message',
          'notification_title',sender_name,'notification_body',coalesce(notification_text,'New message'),
          'target_url','https://medha-hub.web.app/','tag','space-message-'||new.id::text||'-'||recipient
        ))
      );
    end if;
  end loop;
